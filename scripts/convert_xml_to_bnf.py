            ...
            for index, text_token in enumerate(text):
                if answer_tagged:
                    if text_token in dict_tagged.keys():
                        crf_lines.append(
                            [dict_tagged[text_token], extract_pos_from_candidate(text, pos, (index, text_token)),
                             extract_ner_from_candidate(text, ner, (index, text_token)), f'wdm={distance}',
                             extract_question_features(question_qword, question_text, question_qtype, question_pos,
                                                       question_ner, question_headwords, question_comparisons,
                                                       question_delimiters)])
                    else:
                        crf_lines.append([O, extract_pos_from_candidate(text, pos, (index, text_token)),
                                          extract_ner_from_candidate(text, ner, (index, text_token)), f'wdm={distance}',
                                          extract_question_features(question_qword, question_text, question_qtype,
                                                                    question_pos, question_ner, question_headwords,
                                                                    question_comparisons, question_delimiters)])
